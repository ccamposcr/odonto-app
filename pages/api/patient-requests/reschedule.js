...
export default function handler(req, res) {
  ...
  try {
    ...
    const request = db.prepare(`
      ...
      WHERE pr.token = ? AND pr.request_type = 'reschedule' AND pr.status = 'pending'
    `).get(token);
        ...
        <html>
          ...
          <body>
            <div class="container">
              ...
              <p>El enlace de reagendado no es válido, ha expirado o ya fue procesado.</p>
              <p>Si necesita reagendar su cita, por favor contacte directamente a la clínica.</p>
            </div>
          </body>
        </html>
      `);
        ...
        <html>
          ...
          <body>
            <div class="container">
              ...
              <p>El enlace de reagendado ha expirado.</p>
              <p>Por favor contacte directamente a la clínica para reagendar su cita.</p>
            </div>
          </body>
        </html>
      `);
    ...
    db.prepare('UPDATE citas SET estado = ? WHERE id = ?')
      .run('reagendar_solicitado', request.cita_id);
    ...
    res.status(200).send(`
      <html>
        <head>
          <title>Solicitud de Reagendado Recibida - Clínica Dental</title>
          <style>
            body { 
        ...
        <body>
          <div class="container">
            <div class="logo">🦷</div>
            <h1 class="success">Solicitud de Reagendado Recibida</h1>
            
            <p>Estimado/a <span class="highlight">${request.paciente}</span>,</p>
            
            <p>Hemos recibido su solicitud de reagendado para la siguiente cita:</p>
            
            <div class="appointment-card">
              <h3>📅 Cita a Reagendar:</h3>
            ...
            <p>El personal de la clínica revisará su solicitud y se comunicará con usted para coordinar una nueva fecha y hora.</p>
